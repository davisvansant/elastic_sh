...
fn post(base_url: Url) -> Request {
    ...
    let url = base_url.join("/index/_shrink/target_index").unwrap();
...
mod tests {
    ...
    #[test]
    fn get_index_shrink_url() {
        let base = Url::parse("http://elasticsearch:9200").unwrap();
        let request = post(base);
        let request_url = request.url();

        assert_eq!(request_url.as_str(), "http://elasticsearch:9200/index/_shrink/target_index");
        assert_eq!(request_url.path(), "/index/_shrink/target_index");
    }

    #[tokio::test]
    async fn index_shrink_post() {
        let base = Url::parse("http://elasticsearch:9200").unwrap();
