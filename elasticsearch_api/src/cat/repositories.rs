...
fn get(base_url: Url) -> Request {
    ...
    let url = base_url.join("/_cat/repositories").unwrap();
...
mod tests {
    ...
    fn get_repositories() {
        ...
        assert_eq!(request_url.as_str(), "http://elasticsearch:9200/_cat/repositories");
        assert_eq!(request_url.path(), "/_cat/repositories");
    }
}
