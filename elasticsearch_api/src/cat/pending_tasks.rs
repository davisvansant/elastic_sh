...
fn get(base_url: Url) -> Request {
    ...
    let url = base_url.join("/_cat/pending_tasks").unwrap();
...
mod tests {
    ...
    fn get_pending_tasks() {
        ...
        assert_eq!(request_url.as_str(), "http://elasticsearch:9200/_cat/pending_tasks");
        assert_eq!(request_url.path(), "/_cat/pending_tasks");
    }
}
